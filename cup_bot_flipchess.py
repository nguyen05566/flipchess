...
class FlipchessJieqiEngine:
    ...
    def get_best_move(self, fen, moves, movetime_ms=2000, searchmoves=None):
        ...
        try:
            # ★ LUÔN dùng `position startpos moves ...` — KHÔNG dùng `position fen`
            # Engine PikaJieQi có bug khi nhận `position fen` với BAG field:
            #   - Có thể trả move cho sai side (black move khi red to move)
            #   - Có thể segfault
            # Bot track moves qua record_move() → gửi startpos + moves list
        ...
        try:
            ...
            pass
        t0 = time.time()
        timeout = 8.0  # tăng từ 5s → 8s cho engine có thời gian finalize search
        while time.time() - t0 < timeout:
...
class JieqiCupBot:
    def __init__(self):
        ...
        self._rejected_flip_pos = set()  # ★ Track các pos flip bị reject (đã reveal)
    ...
    def _handle_start_match(self, msg):
        ...
        self._bot_move_count = 0  # ★ Reset bot move count cho ván mới
        self._thinking = False
        self._played_this_turn = False
        self._turn_started_at = 0.0
        self._turn_deadline = 0.0
        self._play_reject_count = 0
        self._rejected_moves = set()
        self._rejected_flip_pos = set()  # ★ Reset flip reject tracker
        self._last_sent_move = None
        self._reconnect_streak = 0
        self._enter_fail_at = 0.0
        self._sit_alone_since = None
    ...
    def _handle_play_response(self, msg):
        ...
        if status != 0:
            ...
            if self._last_sent_move:
                ...
                # ★ Track flip reject: nếu last_sent_move là flip (source==target)
                # → pos đã reveal, không flip lại nữa
                if len(self._last_sent_move) == 4 and self._last_sent_move[:2] == self._last_sent_move[2:]:
                    # Flip move: parse UCI → server pos
                    try:
                        uci_sq = self._last_sent_move[:2]
                        # Invert _pos_to_uci: UCI → server pos
                        s_col = ord(uci_sq[0]) - ord('a')
                        uci_rank = int(uci_sq[1])
                        flip = self.board.flip
                        fen_row = 9 - uci_rank
                        s_row = (9 - fen_row) if flip else fen_row
                        reject_pos = s_row * 9 + s_col
                        self._rejected_flip_pos.add(reject_pos)
                        print(f"[FLIP] ⚠️ pos {reject_pos} đã reveal, không flip lại", flush=True)
                    except Exception:
                        pass
    ...
    def _handle_kick_response(self, msg):
        ...
        self._table_path = None; self._table_created_by_me = False
        self._sit_alone_since = None
        self.board.reset()
        # ★ Sau khi bị kick, reset bet_amts để retry tạo bàn mới
        self._bet_amts_loaded = False
        self._resolved_bet_id = None
        # ★ Đợi 5s rồi retry tạo bàn (tránh spam server)
        self._last_quick_play_time = time.time() + 5.0
        print("[KICK] 🔄 Sẽ retry tạo bàn mới sau 5s...")
    ...
    def _handle_gameover(self, msg):
        ...
        def after_gameover():
            print("[GAME] 🔄 Ở lại bàn, sẵn sàng cho ván mới (KHÔNG rời bàn)...")
            time.sleep(3.0)
            if self.connected and not self.board.is_playing:
                # ★ Reset bot move count cho ván mới
                self._bot_move_count = 0
                self._rejected_moves.clear()
                self._play_reject_count = 0
                self.send_ready(1)
                # ★ Nếu bị kick hoặc bàn lock, retry tạo bàn mới
                time.sleep(5.0)
                if not self.in_game and self.connected:
                    print("[GAME] 🔄 Bàn bị lock, retry tạo bàn mới...")
                    self._bet_amts_loaded = False
                    self._resolved_bet_id = None
                    self._last_quick_play_time = 0  # cho phép tạo bàn ngay
    ...
    # ★ Helper: convert server pos → UCI square theo engine coords
    # ★ QUAN TRỌNG: engine startpos có RED ở rank 0, BLACK ở rank 9
    # Khi bot flip=True: server pos 0 (black Rook) → engine rank 9 → UCI 'a9'
    # Khi bot flip=False: server pos 0 (red Rook) → engine rank 0 → UCI 'a0'
    # Logic: server_row → fen_row (qua flip) → engine rank = 9 - fen_row → UCI rank
    @staticmethod
    def _pos_to_uci(pos, flip):
        s_row = pos // 9
        s_col = pos % 9
        # fen_row: theo engine coords (red ở rank 0)
        # flip=True: server row 0 = black side = engine rank 9
        # flip=False: server row 0 = red side = engine rank 0
        fen_row = (9 - s_row) if flip else s_row
        uci_rank = 9 - fen_row
        return f"{chr(ord('a') + s_col)}{uci_rank}"

    # ★ Generate Rook moves (4-char UCI, không có suffix)
    # Sẽ dùng cùng với `position fen <visible_fen>` để engine biết dark piece là Rook
    # Chỉ generate cho quân có raw_face = Rook (cells[pos] == 'R' hoặc 'r')
    def _generate_rook_moves(self):
        cells = self.visible_board.cells
        is_red = self.board.is_red
        flip = self.board.flip
        moves = []

        # ★ Chỉ Xe (R/r)
        target_types = {'R'} if is_red else {'r'}

        for pos in range(90):
            ch = cells[pos]
            if ch not in target_types:
                continue
            r, c = pos // 9, pos % 9

            # ★ Rook moves: đi thẳng, không nhảy qua quân
            # KHÔNG include flip move (source==target) — engine không accept flip trong searchmoves
            for dr, dc in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
                nr, nc = r + dr, c + dc
                while 0 <= nr < 10 and 0 <= nc < 9:
                    target = cells[nr * 9 + nc]
                    if target == '.':
                        src_uci = self._pos_to_uci(pos, flip)
                        tgt_uci = self._pos_to_uci(nr * 9 + nc, flip)
                        moves.append(src_uci + tgt_uci)
                    else:
                        if (target.isupper() != is_red):
                            src_uci = self._pos_to_uci(pos, flip)
                            tgt_uci = self._pos_to_uci(nr * 9 + nc, flip)
                            moves.append(src_uci + tgt_uci)
                        break
    ...
    # ★ Build jieqi FEN từ visible_board: dark pieces → X/x, revealed → true FEN char
    # Engine nhận FEN này sẽ biết dark piece nào là Rook thật (raw_face)
    # → engine generate đúng legal Rook moves cho dark pieces
    def _build_visible_jieqi_fen(self):
        cells = self.visible_board.cells
        is_red = self.board.is_red
        flip = self.board.flip
        dark = self.board.dark_positions

        # Build 10 rows: rank 9 (top) → rank 0 (bottom)
        # Engine coords: rank 0 = bottom (red), rank 9 = top (black)
        rows_fen = []
        for engine_rank in range(9, -1, -1):  # 9 → 0
            server_row = (9 - engine_rank) if flip else engine_rank
            fen_str = ""
            empty = 0
            for col in range(9):
                server_pos = server_row * 9 + col
                ch = cells[server_pos] if 0 <= server_pos < 90 else '.'
                # ★ Dark piece → X (red) hoặc x (black), KHÔNG dùng true piece type
                # Engine hiểu dark piece là "unknown" → accept moves với suffix reveal
                if server_pos in dark and ch != '.':
                    # X = red dark, x = black dark
                    piece_char = 'X' if ch.isupper() else 'x'
                    if empty > 0:
                        fen_str += str(empty)
                        empty = 0
                    fen_str += piece_char
            ...
            if empty > 0:
                fen_str += str(empty)
            rows_fen.append(fen_str)
        return ' '.join([
            '/'.join(rows_fen),
            self.board.side_to_move,
            self.board.bag_string(),
            '0',  # halfmove
            '1',  # fullmove
        ])

    # ★ Tìm dark Rook positions của bot (chưa reveal)
    # Bot biết raw_face qua ws_frame_dump → cells[pos] = 'R' hoặc 'r'
    # dark_positions chứa các pos chưa reveal
    def _find_dark_rooks(self):
        cells = self.visible_board.cells
        is_red = self.board.is_red
        dark = self.board.dark_positions
        target = 'R' if is_red else 'r'
        rooks = []
        for pos in range(90):
            if cells[pos] == target and pos in dark:
                rooks.append(pos)
        return rooks
    ...
    def _do_auto_move(self):
        ...
        # ★ STRATEGY v5: FLIP ROOK TRƯỚC, ENGINE SAU
        # 1. Tìm dark Rook của bot (chưa reveal) — exclude các pos đã reject
        # 2. Nếu có dark Rook → flip nó (source==target) để reveal
        # 3. Nếu flip fail hoặc không có dark Rook → engine full search
        dark_rooks = self._find_dark_rooks()
        # Loại các pos đã bị reject (đã reveal rồi hoặc không flip được)
        dark_rooks = [p for p in dark_rooks if p not in self._rejected_flip_pos]
        if dark_rooks:
            rook_pos = dark_rooks[0]
            uci_sq = self._pos_to_uci(rook_pos, self.board.flip)
            print(f"[FLIP v5] Bot move #{self._bot_move_count + 1}: "
                  f"flip Rook at pos {rook_pos} (UCI {uci_sq})", flush=True)
            ...
            print(f"-> Flip: {uci_sq}{uci_sq} (pos {rook_pos}->{rook_pos}) "
                  f"| uci={len(self.board.uci_moves)}", flush=True)
            self._last_sent_move = uci_sq + uci_sq
            self.send_play(rook_pos, rook_pos)
            self._bot_move_count += 1
            return

        # ★ Sau khi Rook reveal hết (hoặc không flip được) → engine full search
        movetime_ms = 4000 if remain >= 15.0 else (3000 if remain >= 8.0 else 1500)
        if remain < 8.0:
            print(f"[TURN] Sắp hết giờ, movetime={movetime_ms}ms", flush=True)
        ...
        if not raw or (raw and len(raw.split()) >= 2 and raw.split()[1] in self._rejected_moves):
            print("[ENGINE] -> bestmove fail/rejected, restart + retry...", flush=True)
        ...
        if best_move in self._rejected_moves:
            print(f"[ENGINE] Rejected move {best_move}, restart + retry", flush=True)
            if self.engine.restart():
                raw2 = self.engine.get_best_move(fen, moves, movetime_ms=movetime_ms)
                if raw2 and len(raw2.split()) >= 2:
                    best_move = raw2.split()[1]
                    print(f"[ENGINE-OUT] retry bestmove: {best_move}", flush=True)
                else:
                    return
            else:
                return
        if best_move in self._rejected_moves:
            print(f"[ENGINE] Vẫn bị reject — bỏ lượt", flush=True)
            return
        if best_move in ("(none)", "0000"):
            print("[ENGINE] (none) — restart + retry...", flush=True)
            if self.engine.restart():
                raw3 = self.engine.get_best_move(fen, moves, movetime_ms=movetime_ms)
                if raw3 and len(raw3.split()) >= 2:
                    best_move = raw3.split()[1]
                    print(f"[ENGINE-OUT] retry bestmove: {best_move}", flush=True)
                    if best_move in ("(none)", "0000"):
                        self.board.is_my_turn = False
                        return
