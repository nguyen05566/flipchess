...
class FlipchessJieqiEngine:
    ...
    def get_best_move(self, fen, moves, movetime_ms=2000, searchmoves=None):
        ...
        try:
            # ★ Nếu moves=[] rỗng và fen có BAG field → dùng `position fen <fen>`
            # (cho opening strategy với visible FEN đã reveal dark pieces)
            # Ngược lại: dùng `position startpos moves ...` (như cũ)
            if not moves and ' ' in fen and len(fen.split()) >= 4:
                # FEN có BAG field → dùng position fen
            ...
            # Used for opening strategy (3 nước đầu chỉ chọn xe)
            # ★ Accept 4-char moves (chuẩn UCI) — engine không accept 5-char trong searchmoves
            go_cmd = "go infinite"
            if searchmoves:
...
class JieqiCupBot:
    ...
    def _do_auto_move(self):
        ...
        if remain < 4.0:
            ...
            return
        # ★ Smart movetime: sâu hơn khi nhiều thời gian, nhanh hơn khi sắp hết giờ
        # - remain >= 15s: movetime=4000ms (engine nghĩ sâu hơn)
        # - remain 8-15s: movetime=3000ms
        # - remain 4-8s: movetime=1500ms (đi nhanh để không bị hết giờ)
        if remain >= 15.0:
            movetime_ms = 4000
        elif remain >= 8.0:
            movetime_ms = 3000
            ...
            movetime_ms = 1500
            print(f"[TURN] Sắp hết giờ, movetime giảm xuống {movetime_ms}ms", flush=True)
        fen, moves = self.board.get_current_fen()

        # ★ OPENING STRATEGY v4: 3 nước đầu CHỈ chọn XE (R) thật
        # Tận dụng ws_frame_dump biết raw_face → chỉ generate Rook moves cho quân
        # có raw_face = Rook (Xe thật, không phải "xe giả" ở vị trí xe)
        # ★ FIX v4: dùng `position fen` với state đã reveal + searchmoves 4-char
        #   (engine biết dark piece là Rook → accept 'a0a2' trong searchmoves)
        # Sau 3 nước → để engine tự do suy nghĩ
        searchmoves = None
        use_visible_fen = False
        if self._bot_move_count < 3:
            try:
                rook_moves = self._generate_rook_moves()
                if rook_moves:
                    # Loại các nước đã bị server reject trước đó
                    if self._rejected_moves:
                        rook_moves = [m for m in rook_moves if m not in self._rejected_moves]
                    if rook_moves:
                        searchmoves = rook_moves
                        use_visible_fen = True  # ★ Dùng FEN đã reveal cho opening
                        print(f"[OPENING v4] Bot move #{self._bot_move_count + 1}/3: "
                              f"restrict to {len(rook_moves)} ROOK moves (visible FEN)", flush=True)
                else:
                    print(f"[OPENING v4] Bot move #{self._bot_move_count + 1}/3: "
                          f"không có Rook thật nào → full search", flush=True)
            except Exception as e:
                print(f"[OPENING v4] ⚠️ generate_rook_moves lỗi: {e} — full search")
                searchmoves = None

        # ★ Khi opening active: dùng startpos + moves + searchmoves 4-char
        # (engine chỉ accept searchmoves với startpos, không accept với position fen)
        # Bot generate Rook moves theo engine coords (red rank 0, black rank 9)
        if searchmoves:
            print(f"[ENGINE-IN] FEN: {fen[:80]}...", flush=True)
            print(f"[ENGINE-IN] moves({len(moves)}), movetime={movetime_ms}ms, remain={remain:.1f}s"
                 f", searchmoves={len(searchmoves)}",
                 flush=True)
            ...
            print(f"[ENGINE-IN] moves({len(moves)}), movetime={movetime_ms}ms, remain={remain:.1f}s",
                 flush=True)
            raw = self.engine.get_best_move(fen, moves, movetime_ms=movetime_ms)
        # ★ Fallback: nếu Rook restrict không ra bestmove, thử full search
        if not raw and searchmoves:
            print("[ENGINE] -> Rook restrict fail, fallback full search...", flush=True)
            raw = self.engine.get_best_move(fen, moves, movetime_ms=movetime_ms,
                                             searchmoves=None)
        # ★ Khi bị reject: restart engine để có state sạch, rồi retry
        ...
        if best_move in self._rejected_moves:
            print(f"[ENGINE] Rejected move {best_move}, restart + retry với state sạch", flush=True)
            if self.engine.restart():
        ...
        if best_move in ("(none)", "0000"):
            print("[ENGINE] (none) — engine không tìm thấy nước hợp lệ, thử restart...", flush=True)
            # ★ Khi engine trả (none), restart + retry một lần cuối
            if self.engine.restart():
        ...
        try:
            ...
            self._bot_move_count += 1  # ★ Tăng counter sau khi bot đi
        except Exception as e:
            print(f"[BOT ERROR] {e}")
            traceback.print_exc()
